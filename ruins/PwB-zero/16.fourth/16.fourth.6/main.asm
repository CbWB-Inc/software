...
k_task_sector  equ 19       ; 子1はセクタ10から読み込む
...
setup:
    ...
    ;mov di, shared_head_ofs
    ;mov word ax, [di + 0]
    ;call disp_hex

    ;mov di, shared_tail_ofs
    ;mov word ax, [di + 0]
    ;call disp_hex

    ;mov di, shared_data_ofs
    ;mov word [di + 0], 1
    ;mov word ax, [di + 0]
    ;call disp_hex
...
init_shared_log:
    ...
    mov bx, shared_head_ofs
    mov [es:bx], ax
    
    mov ax, shared_buf_len - 1
...
;    mov bx, 0x100
    ...
    ;mov al, ' '
    ;call putc
    ;mov word [es:bx + 0], 0x0100
    ;mov word ax, [es:bx + 0]
    ;call disp_hex
    ;mov al, ' '
    ;call putc
    ;mov word [es:bx + 2], 0x0200
    ;mov ax, [es:bx + 2]
    ;call disp_hex
    ;mov al, ' '
    ;call putc
    ;mov word [es:bx + 4], 0x0400
    ;mov ax, [es:bx + 4]
    ;call disp_hex
    ;mov al, ' '
    ;call putc
    ;mov word [es:bx + 6], 0x0600
    ;mov ax, [es:bx + 6]
    ;call disp_hex
    ;mov al, ' '
    ;call putc
    ;mov word [es:bx + 8], 0x0800
    ;mov ax, [es:bx + 8]
    ;call disp_hex
...
write_log_safe:
    ...
    ; head / tail の取得
    ;mov bx, [shared_head_ofs]
    ;mov cx, [shared_tail_ofs]

    mov bx, [es:shared_head_ofs]
    mov cx, [es:shared_tail_ofs]

    ; 空きチェック： (head + 1) % len == tail → フル
    ;mov dx, bx
    ;inc dx
    ;and dx, shared_buf_len - 1
    ;cmp dx, cx
    ;je .full      ; フル → 書き込みせずret

    inc bx
    and bx, shared_buf_len - 1
    ...
    ;inc bx
    ;and bx, shared_buf_len - 1
    ;mov [shared_head_ofs], bx
    mov [es:shared_head_ofs], bx

    
    ; 書き込み先 = shared_data_ofs + head
    mov dx, shared_data_ofs
    add dx, bx
    mov di, dx
    mov [es:di], al
...
write_log3:
    ...
    inc bx
    mov [es:shared_head_ofs], bx
    ;add bx, shared_data_ofs - 1

    ;mov [es:shared_data_ofs + bx -1], al
.full:
