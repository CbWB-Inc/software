...
write_log:
    ...
    ; 現在のhead位置を取得
    mov word bx, [shared_head_ofs]
    mov di, bx
    
    ; 次のhead位置を計算（バッファサイズ6に対応）
    mov cx, bx
    inc cx
    cmp cx, shared_buf_len      ; shared_buf_lenの実際の値
    jb .no_wrap
    xor cx, cx                  ; バッファ末尾に達したら0に戻す
.no_wrap:
    
    ; バッファが満杯かチェック
    ;cmp cx, [es:shared_tail_ofs]
    cmp cx, [di + shared_tail_ofs]
    je .full                    ; 次の位置がtailと同じなら満杯
    
    ; 現在のhead位置にデータを書き込み
    ;add bx, shared_data_ofs     ; オフセットを加算
    ;mov byte [es:bx], al             ; 書き込み
    ;mov bx, [di + shared_head_ofs]
    mov [di + shared_data_ofs + bx], al
    ;sub bx, shared_data_ofs     ; 元に戻す
    
    ; head位置を更新
    ;mov word [es:shared_head_ofs], cx
    mov word [di + shared_head_ofs], cx
...
write_log2:
    ...
    mov cx, bx
    inc cx
    cmp cx, 6                   ; shared_buf_lenの実際の値
    jb .no_wrap
    xor cx, cx                  ; バッファ末尾に達したら0に戻す
.no_wrap:
    
    ; バッファが満杯かチェック
    cmp cx, [shared_tail_ofs]
    je .full                    ; 次の位置がtailと同じなら満杯
