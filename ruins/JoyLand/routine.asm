...
._loop_exit: 
    ...
    mov ax, ._s_indata_hdr
    call disp_str
    mov ax, ._s_test_data
    call disp_str
    call disp_nl

    mov ax, ._s_expect_hdr
    call disp_str
    mov ax, ._s_expect
    mov bx, 0x05
    call disp_mem

    mov ax, ._s_result_hdr
    call disp_str
    mov ax, ._s_buf
    mov bx, 0x05
    call disp_mem
...
._s_test_data: db 0x30, 0x39, 0x41, 0x46, 0x61, 0x66, 0x39, 0x46, 0x67, 0x00
._s_expect: db 0x09, 0xAF, 0xAF, 0x9F, 0x00
._s_buf: times 255 db 0x00
._s_success: db 'SUCCESS!! (^^)b', 0x0d, 0x0a, 0x00
._s_fail: db 'fail (T_T)', 0x0d, 0x0a, 0x00

._s_indata_hdr: db 'indata : ', 0x00
._s_expect_hdr: db 'expect : ', 0x00
._s_result_hdr: db 'result : ', 0x00
...
._s_test_data: db 0x30, 0x39, 0x41, 0x46, 0x61, 0x66, 0x39, 0x46, 0x67, 0x00
._s_expect : db 0x00, 0x09, 0x0A, 0x0F, 0x0A, 0x0F, 0x09, 0x0F, 0x00 
._s_success: db 'SUCCESS!! (^^)b', 0x0d, 0x0a, 0x00
._s_fail: db 'fail (T_T)', 0x0d, 0x0a, 0x00
._s_buf: times 255 db 0x00
...
;	（ビッグエンディアン表記）
; param : ax : 表示するword
; return : bx : 変換した文字列4バイトを格納するアドレス
;********************************
bin_word_hex:
