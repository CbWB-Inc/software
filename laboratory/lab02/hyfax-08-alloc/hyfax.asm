...
%define PART_TBL_OFF    0xF000
%define BPB_ADDR        0xF100
%define BOOT_DRV_OFF    0xF040
%define VBR_START_LBA   0xF042
%define FAT_START_LBA   0xF046
%define DIR_START_LBA   0xF04A
%define DATA_START_LBA  0xF04E
%define SEC_PER_CLUS_OFF    0xF050
%define CLUS_SIZ_OFF     0xF052
%define BYTE_PER_SEC_OFF 0xF05D
%define ROOT_ENT_CNT_OFF 0xF060
%define PART_TBL_OFF 446
%define PART_ENT_SZ  16
%define VBR_SECTORS 1
%define ENT_SECTORS 1
%define MHS_OFF 0xF070
%define FATHS_OFF 0xF100
...
%define svc_exec    0x27
%define svc_putphs  0x28
%define svc_page_alloc  0x30
%define svc_page_free   0x31
%define svc_page_dump   0x29
...
; ; ---- globals (Monitor常駐) ----
; g_managed_end  dd 0        ; 管理終端（4KB align済） (phys addr)
; g_num_pages    dw 0        ; 管理ページ数（MANAGED_BASE からの相対: index 0..)
; g_scan_hint    dw 0        ; 次に探し始める index
; bitmap_seg dw 0

; bitmap は MANAGED_BASE に直置き（page0 先頭）
; bitmap bit i == 1 => page index i が使用中
...
;===============================
; FAT Handle Structure
;===============================
struc FATHS
    .Current_Data_LBA   resd 1
    .Parent_Data_LBA    resd 1
    .Path_LBA           resd 64   ; ancestor stack
    .Path_Depth         resd 1
    .file_lba           resd 1    ; file lba
    .file_size          resd 1    ; file size (bytes)
    .file_name          resb 11   ; 8.3 short name
                        resb 1
    .buf_ptr            resd 1
    .buf_size           resd 1
    .pos                resd 1
...
struc MHS
    .AXs   resd 64
    .DXs   resd 64
    .cnt   resd 1      ; stack depth
endstruc



; ==============================
; Fixed layout (NO struc)
; ==============================

; ---- DHS (Digit Handle Structure) ----
DHS_VAL   equ 0              ; byte[40]
DHS_LEN   equ 40             ; word
DHS_SIGN  equ 42             ; byte
DHS_SIZE  equ 43

; ---- PHS (Packed Handle Structure) ----
PHS_VAL   equ 0              ; byte[20]
PHS_LEN   equ 20             ; word
PHS_SIGN  equ 22             ; byte
PHS_SIZE  equ 23