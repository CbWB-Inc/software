...
find_dir_ent:
    ...
    mov ax, es:[di+28]
    mov [FILE_SIZE], ax
    mov es:[si + FATHS.file_size],ax
    mov ax, [FILE_LBA]
    mov es:[si + FATHS.file_lba],ax
    mov ax, [DATA_LBA]
    mov es:[si + FATHS.Current_Data_LBA], ax
    mov word es:[si + FATHS.Parent_Data_LBA], 0
    ; mov ax, cs
    ; mov ds, ax

    mov ax, TARGET_APP_NAME
...
.alloc_loop:
    ...
    push bx
    shl bx, 1
    mov [si + MHS.AXs + bx], ax
    mov [si + MHS.DXs + bx], dx
    pop bx
    inc bx    
    loop .alloc_loop
...
runapp:
    ...
    ; monitor のグローバル変数から引数を取得
    ; この時点で DS = MON_SEG のはず
    ; PUTC 'L'
    ; PUTC 'D'
    ; PUTC '='
    ; push ax
