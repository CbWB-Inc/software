...
void kernel_main(handoff_t* info) {
    ...
    // 例外が拾えるようになったので試しに例外を起こしてみる
    
    // #UD: invalid opcode
    // __asm__ volatile("ud2");

    // #DE: divide by zero（volatileで最適化回避）
    // volatile int z = 0, a = 1;
    // a = a / z;

    // #PF: 未マップ領域アクセス（あなたの張り方なら2GiBは未マップのはず）
    // *(volatile uint64_t*)0x0000000080000000ull = 1;
...
void setup_gdt(void) {
    ...
    gdt_mem[3] = 0x0000000000000000;         // 未使用
