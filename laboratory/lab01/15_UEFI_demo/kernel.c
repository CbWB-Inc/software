...
void kernel_main(handoff_t* info) {
    ...
    // マウスカーソルをフレームバッファに設定
    extern uint32_t _pitch;
    mouse_cursor_set_fb(fb, _pitch * 4, 1280, 800, 1);
    ...
    lapic_base = (uint32_t*)lapic_phys;
    ...
    ps2_mouse_init_min();
     ...
     // キーボード・マウス割り込み設定
    ps("Setting up keyboard and mouse interrupt...\n");
    ...
    // スケジューラーの初期化と開始
    ps("Initializing scheduler...\n");
    start_scheduler();

    // タイマー開始とキャリブレーション
    ps("Starting timer...\n");
    lapic_enable_and_start_timer();
    lapic_timer_set_1khz_calibrated();
    
    ps("Enabling interrupts...\n");
    
    // 割り込みを開放（開放すると皇族処理がブロックされます。）
...
void init_paging(void* page_table_base, uint64_t framebuffer_addr) {
    ...
    for (uint64_t offset = 0; offset < fb_size; offset += 0x1000) {
        ...
        if (offset < 0x200000)
            ...
            pt_fb3[page_idx] = (framebuffer_addr + offset) | PAGE_PRESENT | PAGE_RW | PAGE_PWT | PAGE_PCD;
        else
            pt_fb4[page_idx] = (framebuffer_addr + offset) | PAGE_PRESENT | PAGE_RW  | PAGE_PWT | PAGE_PCD;
    }
    // ページディレクトリエントリを設定
    ...
    pd[4] = (uint64_t)(uintptr_t)pt_fb3 | PAGE_PRESENT | PAGE_RW  | PAGE_PWT | PAGE_PCD;     // 6MB-8MB（フレームバッファ）
    pd[5] = (uint64_t)(uintptr_t)pt_fb4 | PAGE_PRESENT | PAGE_RW  | PAGE_PWT | PAGE_PCD;     // 6MB-8MB（フレームバッファ）
