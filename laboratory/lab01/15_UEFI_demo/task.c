...
#define TASK_DEMO1_WEIGHT 2
#define TASK_DEMO2_WEIGHT 1
...
void __attribute__((naked)) task_D_entry(void) {
    asm volatile(
        "jmp task_D"
    );
}
...
void __attribute__((optimize("-fomit-frame-pointer"))) task_A(void) {
    ...
    for (int i = 0; i < 0xffff; i++) {
        ...
        uint64_t sx = 80;
        uint64_t sy = 4 * (2 - 1);
         psfb("task_A", sx*8, sy*16, _color);

        psfb(" : 0x", (sx+6)*8, sy*16, _color);
        ph4fb(live_count++, (sx+11)*8, sy*16, _color);
        _c_x = x;_c_y = y;
...
// シンプルなタスクD（フレームポインタなし）
void __attribute__((optimize("-fomit-frame-pointer"))) task_D(void) {
    //    ps("=== Task D started ===\n");
    uint64_t live_count = 0;
    uint8_t buf[128];
...
//    ps("  task_entry: "); ph((uint64_t)task_entry); ps("\n");
    ...
    // メモリ範囲チェック
    if (!is_mapped((uint64_t)sp) || !is_mapped((uint64_t)sp - 512)) {
    ...
    *(--sp) = 0x0000000000000000ULL;     // R15
    ...
    if (!is_mapped((uint64_t)task_stack_base) || !is_mapped((uint64_t)(task_stack_base - 512))) {
        ps("ERROR: Task stack base not mapped: "); ph(task_stack_base); ps("\n");
        return NULL;
    }

    // 最終チェック - 割り込みスタックフレーム全体をチェック
    if (!is_mapped((uint64_t)sp) || !is_mapped((uint64_t)sp + 20)) {
...
int init_scheduler(void) {
    ...
    taskC->rip = (uint64_t*)task_C;
    taskC->cond = TASK_RUNNING;

    task_t* taskD = create_task(task_D_entry, 2, "task_D");  
    if (!taskD) {
        ps("ERROR: Failed to create task D\n");
        return -1;
    }
    // ps("✓ Task C created\n");
    taskD->kind = TASK_NORMAL;
    taskD->rip = (uint64_t*)task_D;
    taskD->cond = TASK_RUNNING;

    // idle, taskA, taskB を作った直後に
    set_task_weight(idle,  0);                // idle は直ちに手放す挙動に
    set_task_weight(taskA, TASK_SHELL_WEIGHT);    // 比率 3
    set_task_weight(taskB, TASK_DEMO1_WEIGHT);    // 比率 2 （デモ用）
    set_task_weight(taskC, TASK_DEMO2_WEIGHT);    // 比率 3 （デモ用）
    set_task_weight(taskD, TASK_SHELL_WEIGHT);    // 比率 1 
...
void* sched_tick_isr(void* saved_rsp) {
    ...
    if (rsp_val < 0x1000000 || rsp_val > 0x80000000 || !is_mapped((uint64_t)saved_rsp)) {
    ...
    if (!is_mapped((uint64_t)to_run->rsp) || !is_mapped((uint64_t)(to_run->rsp + 19))) {
    ...
    // RIPの妥当性チェック
    if (rip < 0x100000 || rip > 0x200000 || !is_mapped(rip)) {
